...
Перед оновленням до 3.6 переконайтеся, що [всі ваші учасники 3.5 оновлені до 3.5.32 або пізнішої версії](/blog/2026/july-patch-release/). Патч-випуски 3.5.24–[3.5.32](/blog/2026/july-patch-release/) виправляють декілька потенційних блокерів оновлення, включаючи `--v2-deprecation=write-only-skip-check` та `etcdutl check v2store` для перевірки записів WAL, а також знімка v2.
...
Якщо прапорець --enable-v2 та змінна середовища ETCD_ENABLE_V2 ніколи не були увімкнені (тобто завжди були не встановлені або встановлені в false) на будь-якому учаснику з моменту створення кластера, оновлення до etcd v3.6 є безпечним.

Перед оновленням завжди переконайтеся, що прапорець --enable-v2 та змінна середовища ETCD_ENABLE_V2 видалені або не встановлені на всіх учасниках etcd, оскільки вони більше не підтримуються в etcd v3.6.
{{% /alert %}}

Якщо `--enable-v2` або змінна середовища `ETCD_ENABLE_V2="true"` була (або коли-небудь була) налаштована на будь-якому учаснику, сховище v2store повинно бути очищене перед тим, як кластер можна буде безпечно оновити до v3.6. Загальний робочий процес:

1. Виконайте команду `etcdutl check v2store` на кожному учаснику, щоб перевірити, чи містить v2store якісь дані, що не стосуються членства (дані користувача). Переконайтеся, що ви використовуєте `etcdutl` версії 3.5.32 або пізнішої, яка розширює перевірку, додаючи до знімка v2 також перевірку записів WAL.

2. Перевірка повідомляє один із наступних результатів. Усуньте будь-які проблеми, виявлені перевіркою, як описано у відповідних розділах.

    - `No custom content found in both v2store and WAL records` — сховище v2store чисте. Перейдіть до кроку 4.
    - `detected custom content in v2store` — кастомні дані v2 існують лише у знімку v2. Дотримуйтесь інструкцій у розділі [v2store містить кастомні дані](#v2store-has-custom-data).
    - `detected custom v2 content in WAL records` — кастомні дані v2 існують лише в записах WAL. Дотримуйтесь інструкцій у розділі [WAL містить кастомні дані v2](#wal-has-custom-v2-data).
    - `detected custom v2 content in both v2store and WAL records` — кастомні дані v2 існують в обох місцях. Дотримуйтесь інструкцій у розділах [v2store містить кастомні дані](#v2store-has-custom-data) та [WAL містить кастомні дані v2](#wal-has-custom-v2-data).
    - Щось інше — цей результат зазвичай не повинен відбуватися; якщо відбувається, перевірка могла видати неочікуваний вивід, тому зупиніться та розслідуйте вручну.

3. Повторіть крок 1, щоб перевірити, що v2store чисте. Якщо перевірка все ще повідомляє про кастомні дані, поверніться до кроку 2 та усуньте решту проблем.

4. Щойно `etcdutl check v2store` повідомить "No custom content found in both v2store and WAL records" на кожному учаснику, видаліть прапорець `--enable-v2` та змінну середовища `ETCD_ENABLE_V2` з усіх учасників etcd.

    **Готово! Тепер ви можете безпечно оновити etcd до v3.6!**

##### v2store містить кастомні дані {#v2store-has-custom-data}

Якщо вам потрібні кастомні дані v2 або вам потрібно мігрувати їх до v3store, дотримуйтесь [посібника з міграції v2](/docs/v3.4/op-guide/v2-migration/) для міграції даних.

Якщо вам не потрібні кастомні дані v2, ви можете:

- Видалити кастомні дані v2 вручну з теки даних, або
- Запустити etcd з прапорцем `--v2-deprecation=write-only-skip-check`, щоб перевірка v2store була пропущена під час запуску. Зверніть увагу, що це потребує `etcd` версії v3.5.32 або пізнішої, і ви використовуєте цей параметр на власний ризик.

Після усунення проблеми повторно виконайте `etcdutl check v2store`, щоб підтвердити, що v2store чисте.

##### WAL містить кастомні дані v2 {#wal-has-custom-v2-data}

Кастомні дані v2 у записах WAL автоматично видаляються при створенні наступного знімка v2. За стандартної конфігурації знімків новий знімок v2 створюється після великої кількості зафіксованих транзакцій. Точний час очікування залежить від частоти записів вашого кластера; у завантажених кластерах наступний знімок може бути створений протягом кількох годин, тоді як у менш активних кластерах це може зайняти більше часу.

- **Зачекайте на наступний знімок v2.** Якщо ви можете дочекатися, тримайте кластер запущеним; новий знімок v2 зрештою буде створено, і WAL буде очищено автоматично.

- **Примусово створіть знімок негайно.** Якщо ви не хочете чекати, застосуйте обхідний шлях `--snapshot-count=1` на кожному ураженому учаснику:

    1. Додайте прапорець `--snapshot-count=1` до кожного учасника etcd, який повідомляє про кастомні дані WAL.
    2. Перезапустіть учасників etcd.
    3. Щойно наступний знімок буде створено, видаліть прапорець `--snapshot-count=1` (або відновіть його початкове значення, якщо застосовно).
    4. Перезапустіть учасників etcd ще раз.

Після створення нового знімка v2 повторно виконайте `etcdutl check v2store`, щоб підтвердити, що WAL чистий.
...
Щоб оновити наявне розгортання etcd до v3.6, кластер, що працює, повинен бути версії v3.5 або вище. Якщо це версія до v3.5, спочатку [оновіть до v3.5](/docs/v3.6/upgrades/upgrade_3_4/) перед оновленням до v3.6.
...
Перед початком [завантажте резервну копію знімка](/docs/v3.6/op-guide/maintenance/#snapshot-backup). Якщо щось піде не так під час оновлення, можна використовувати цю резервну копію для [пониження версії](#rollback) до поточної версії etcd. Зверніть увагу, що команда `snapshot` створює резервну копію лише даних v3.
...
[Завантажте резервну копію знімка](/docs/v3.6/op-guide/maintenance/#snapshot-backup), щоб забезпечити шлях для пониження версії у разі виникнення проблем.
