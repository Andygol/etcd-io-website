...
#### Змінено `etcdserver.EtcdServer.ServerConfig` на `*etcdserver.EtcdServer.ServerConfig` {#changed-etcdserveretcdserverserverconfig-to-etcdserveretcdserverserverconfig}
...
#### Додано структуру `embed.Config.LogOutput` {#added-embedconfiglogoutput-struct}
...
Метод `embed.Config.SetupLogging` був визнаний застарілим у v3.4. Будь ласка, дивіться [посібник з оновлення до v3.4](/docs/{{< param version >}}/upgrades/upgrade_3_4/) для отримання додаткової інформації.**
...
Зверніть увагу, що у версіях v3.3.0-rc.0, v3.3.0-rc.1 та v3.3.0-rc.2, `etcdhttp.Health` має поля типу boolean `"health"` та `"errors"`. Для зворотної сумісності ми повернули поле `"health"` до типу `string` і видалили поле `"errors"`. Додаткова інформація про справність буде надана в окремих API.
...
#### Змінено HTTP точки доступу gRPC шлюзу (замінено `/v3alpha` на `/v3beta`) {#changed-grpc-gateway-http-endpoints-replaced-v3alpha-with-v3beta}
...
#### Змінено імпорти `golang.org/x/net/context` {#changed-golangorgxnetcontext-imports}
...
##### Застарілий `grpclog.Logger` {#deprecated-grpcloglogger}
...
##### Застарілий `grpc.ErrClientConnTimeout` {#deprecated-grpcerrclientconntimeout}
...
### Оновлення до >= v3.3.14 {#upgrades-to--v3314}
...
Будь ласка, дивіться [CHANGELOG](https://github.com/etcd-io/etcd/blob/main/CHANGELOG/CHANGELOG-3.3.md) для повного списку змін.

### Контрольні списки для оновлення сервера {#server-upgrade-checklists}

#### Вимоги до оновлення {#upgrade-requirements}

Щоб оновити наявне розгортання etcd до 3.3, кластер, що працює, повинен бути версії 3.2 або вище. Якщо версія нижче 3.2, будь ласка, [оновіть до 3.2](/docs/v3.6/upgrades/upgrade_3_2/) перед оновленням до 3.3.
...
Будь ласка, [зробіть резервну копію теки даних](/docs/v3.6/op-guide/maintenance/#snapshot-backup) всіх членів etcd, щоб зробити можливим пониження версії кластера навіть після його повного оновлення.
...
#### 1. Перевірте вимоги до оновлення {#1-check-upgrade-requirements}
...
#### 2. Зупиніть поточний процес etcd {#2-stop-the-existing-etcd-process}
...
На цьому етапі добре [зробити резервну копію даних etcd](/docs/v3.6/op-guide/maintenance/#snapshot-backup), щоб забезпечити шлях до пониження версії у разі виникнення будь-яких проблем:

```sh
$ etcdctl snapshot save backup.db
```

#### 3. Вставте бінарний файл etcd v3.3 і запустіть новий процес etcd {#3-drop-in-etcd-v33-binary-and-start-the-new-etcd-process}
...
#### 4. Повторіть крок 2 до кроку 3 для всіх інших членів {#4-repeat-step-2-to-step-3-for-all-other-members}

#### 5. Завершення {#5-finish}
