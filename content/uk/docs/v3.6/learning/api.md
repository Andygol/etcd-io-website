...
* `kvs` — список пар ключ-значення, що відповідають запиту діапазону. `kvs` порожній, коли запитується count.
* `more` — вказує, чи є ще ключі для повернення в запитуваному діапазоні.
* `count` — фактична кількість ключів у діапазоні, коли це запитується. На відміну від `kvs`, на нього не впливають обмеження та фільтри (наприклад, Min/Max, Create/Modify, Revisions), і він відображає повну кількість у вказаному діапазоні.
...
* `prev_kv` — коли встановлено, etcd отримує попередню пару ключ-значення перед її зміною. Попередня пара ключ-значення буде повернута у відповіді put.
* `ignore_value` — коли встановлено, оновлює ключ, використовуючи його поточне значення. Повертає помилку, якщо ключ не існує.
* `ignore_lease` — коли встановлено, etcd оновлює ключ, використовуючи його поточну lease. Повертає помилку, якщо ключ не існує.
...
* `prev_kv` — якщо prev_kv встановлено в запиті, попередня пара ключ-значення буде повернута.
...
* `key` — перший ключ для видалення в діапазоні.
* `range_end` — ключ, що слідує за останнім ключем для видалення для діапазону `[key, range_end)`.
...
* `deleted` — кількість ключів, видалених запитом видалення діапазону.
* `prev_kv` — коли встановлено, попередні пари ключ-значення будуть повернуті.
...
[watch-api-guarantees]: /docs/v3.6/learning/api_guarantees/#watch-apis
[elections]: https://github.com/etcd-io/etcd/blob/main/client/v3/concurrency/election.go
[grpc-api]: /docs/v3.6/dev-guide/api_reference_v3/
