...
Нижче показано, як виконати [статичний процес завантаження](/docs/v3.6/op-guide/clustering/#static) як Kubernetes StatefulSet.
...
spec:
  ...
  selector:
    app: etcd
  ##
  ## В ідеалі ми мали б використовувати SRV-записи для виявлення peerʼів для ініціалізації.
...
spec:
  ...
  serviceName: etcd
  ##
  ## Якщо ви збільшуєте кількість реплік наявного кластера, вам слід
  ## також оновити прапорець --initial-cluster-state, як зазначено нижче у
  ## конфігурації контейнера.
  replicas: 3
  ##
  ## Для ініціалізації, podʼи etcd повинні бути доступні один одному до того, як
  ...
  ## були онлайн у будь-який момент часу.
  updateStrategy:
    type: RollingUpdate
  ##
  ...
  template:
    ...
    spec:
      ...
      - name: etcd
        ...
        ## запуск, але безпечно зазнає невдачі.
