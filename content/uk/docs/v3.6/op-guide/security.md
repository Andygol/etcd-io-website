...
etcd підтримує автоматичний TLS, а також автентифікацію через клієнтські сертифікати як для клієнтів до сервера, так і для звʼязку між вузлами (сервер до сервера / кластер).

{{% alert title="Попередження" color="warning" %}}

etcd стандартно не вмикає [автентифікацію на основі RBAC](/docs/v3.6/op-guide/authentication/) або функцію автентифікації на транспортному рівні, щоб зменшити труднощі для користувачів, які починають працювати з базою даних. Крім того, зміна цього стандартного параметра буде порушенням для проєкту, який був заснований у 2013 році. Кластер etcd, який не вмикає функції безпеки, може експонувати свої дані будь-яким клієнтам.
...
Список прапорців, наведений нижче, може бути неактуальним через постійні зміни в розробці. Для отримання останніх доступних прапорців запустіть `etcd --help` або зверніться до [довідки etcd][etcd help].
...
Коли ви вказуєте ClientAutoTLS і PeerAutoTLS, термін дії клієнтського сертифіката та сертифіката вузла, автоматично згенерованого etcd, становить лише 1 рік. Ви можете вказати прапорець --self-signed-cert-validity, щоб встановити термін дії сертифіката в роках.
...
Налаштуйте etcd для використання самопідписних сертифікатів для клієнтських і вузлових зʼєднань за допомогою прапорців `--auto-tls` та `--peer-auto-tls`:
...
## Приклад 5: Перетворення наявного кластера etcd без TLS на кластер з TLS {#example-5-convert-an-existing-non-tls-etcd-cluster-to-tls}

Раніше ми описали, що etcd підтримує два типи звʼязку: **клієнт-сервер** та **сервер-сервер (звʼязок між вузлами)**. У цьому розділі ми підготуємо кроки конфігурації для міграції обох типів звʼязку на TLS у вашому наявному кластері etcd.

{{% alert title="Важливо"%}}

Перед початком роботи з перетворення кластера на TLS, будь ласка, зробіть резервну копію бази даних etcd.

{{% /alert %}}
{{% alert title="Важливо"%}}

Перед перезапуском будь-яких учасників etcd ви повинні спочатку додати TLS-сертифікати та конфігурацію до кожного вузла etcd.

{{% /alert %}}

---

### 1. Перевірка поточного статусу кластера {#1-check-current-cluster-status}

```sh
export ETCDCTL_API=3

# Визначте IP-адреси вузлів etcd
HOST_1=ip_address_etcd_node_1
HOST_2=ip_address_etcd_node_2
HOST_3=ip_address_etcd_node_3

# Отримайте список учасників
etcdctl --endpoints=${HOST_1}:2379,${HOST_2}:2379,${HOST_3}:2379 member list
```

Ви отримаєте вихідні дані, подібні до:
...
{{% alert title="Примітка" color="info" %}}
Збережіть **ID учасників** та відповідні **IP-адреси** — вони знадобляться вам для оновлення URL-адрес вузлів.
{{% /alert %}}

---

### 2. Додавання TLS-сертифікатів на кожен вузол {#2-add-tls-certs-on-each-node}

Будь ласка, додайте TLS-сертифікати до кожного вузла, використовуючи інструмент [cfssl], який надає простий інтерфейс для генерації сертифікатів, і ми надаємо [інструкцію][tls-setup] щодо використання інструменту. Альтернативно, спробуйте цей [посібник з генерації самопідписних пар ключів][tls-guide].

### 3. Увімкнення TLS для звʼязку клієнт-сервер {#3-enable-tls-for-client-to-server-communication}

Оновіть параметри кожного вузла відповідно до вашого методу розгортання etcd, використовуючи наступні прапорці:
...
Також оновіть усі `--listen-peer-urls`, `--listen-client-urls`, `--advertise-client-urls`, `--initial-cluster` та `--initial-advertise-peer-urls`, щоб використовувати `https://`.

### 4. Увімкнення TLS для звʼязку сервер-сервер {#4-enable-tls-for-server-to-server-communication}

Оновіть URL-адресу вузла кожного учасника etcd, щоб використовувати `https://`, крок за кроком:

```sh
$ На вузлі 10f4390f4a904a13
etcdctl member update 10f4390f4a904a13 --peer-urls=https://ip_address_etcd_node_1:2380
$ На вузлі 18e03f3590bf045c
etcdctl member update 18e03f3590bf045c --peer-urls=https://ip_address_etcd_node_2:2380
$ На вузлі ca7a7d6fe0984fd9
etcdctl member update ca7a7d6fe0984fd9 --peer-urls=https://ip_address_etcd_node_3:2380
```

Після цього, будь ласка, перезапустіть службу systemd etcd на кожному вузлі etcd крок за кроком:
...
Чому це важливо? Якщо ми перетворюємо кластер з 3 вузлів один за одним, то на першому етапі ми матимемо 1 вузол з TLS, 2 без TLS, і кластер відповідатиме без TLS. На 2-му етапі ми перетворюємо 2-й вузол, що призводить до кластера з TLS (2 з 3 вузлів з TLS), який також функціонуватиме належним чином.

---

### 5. Перевірка захищеного звʼязку {#5-verify-secure-communication}

Перевірте стан кластера з використанням TLS:
...
Ви повинні побачити відповіді, подібні до:

```sh
https://ip_address_etcd_node_1:2379 is healthy: successfully committed proposal
```

Якщо всі точки доступу повертають стан healthy, ваш кластер тепер повністю захищений за допомогою TLS!
...
Починаючи з [v3.2.5](https://github.com/etcd-io/etcd/blob/main/CHANGELOG/CHANGELOG-3.2.md#v325-2017-08-04), [сервер підтримує зворотний пошук на основі підстановочних знаків DNS `SAN`](https://github.com/etcd-io/etcd/pull/8281). Наприклад, якщо сертифікат вузла містить лише DNS-імена (без IP-адрес) в полі Subject Alternative Name (SAN), сервер спочатку виконує зворотний пошук віддаленої IP-адреси, щоб отримати список імен, що відповідають цій адресі (наприклад, `nslookup IPADDR`). Потім приймає зʼєднання, якщо ці імена мають відповідне імʼя з DNS-іменами сертифіката вузла (або за точним або підстановочним збігом). Якщо жоден збіг не знайдено, сервер виконує прямий пошук кожного DNS-запису в сертифікаті вузла (наприклад, шукає `example.default.svc`, коли запис є `*.example.default.svc`), і приймає зʼєднання лише тоді, коли розвʼязані адреси хоста мають відповідну IP-адресу з віддаленою IP-адресою вузла. Наприклад, CSR вузла B (з `cfssl`) виглядає так:
...
коли віддалена IP-адреса вузла B — `10.138.0.2`. Коли вузол B намагається приєднатися до кластера, вузол A виконує зворотний пошук IP `10.138.0.2`, щоб отримати список імен хостів. І виконує точний або підстановочний збіг імен хостів з DNS-іменами сертифіката B у полі Subject Alternative Name (SAN). Якщо жоден зворотний/прямий пошук не спрацював, він повертає помилку `"tls: "10.138.0.2" does not match any of DNSNames ["*.example.default.svc","*.example.default.svc.cluster.local"]`. Див. [issue#8268](https://github.com/etcd-io/etcd/issues/8268) для отримання додаткової інформації.

[v3.3.0](https://github.com/etcd-io/etcd/blob/main/CHANGELOG/CHANGELOG-3.3.md) додає прапорець [`etcd --peer-cert-allowed-cn`](https://github.com/etcd-io/etcd/pull/8616) для підтримки [автентифікації на основі CN (Common Name) для міжвузлових зʼєднань](https://github.com/etcd-io/etcd/issues/8262). TLS-автентифікація Kubernetes включає генерацію динамічних сертифікатів для учасників etcd та інших системних компонентів (наприклад, API-сервер, kubelet тощо). Підтримка різних сертифікаційних центрів для кожного компонента забезпечує більш жорсткий контроль доступу до кластера etcd, але часто є трудомісткою. Коли вказано прапорець `--peer-cert-allowed-cn`, вузол може приєднатися лише з відповідним загальним імʼям, навіть зі спільними центрами сертифікації. Збіг виконується як точне порівняння рядків із полем Common Name (CN) сертифіката — підстановочні знаки або збіг за префіксом не підтримуються. Для фільтрації на основі імен хостів за допомогою `--peer-cert-allowed-hostname` або `--client-cert-allowed-hostname` збіг використовує `x509.Certificate.VerifyHostname()` з Go, який підтримує як точні імена хостів, так і записи з підстановочними знаками (наприклад, `*.example.com`). Наприклад, кожен учасник у кластері з 3 вузлів налаштований з CSR (з `cfssl`) наступним чином:
...
[v3.2.19](https://github.com/etcd-io/etcd/blob/main/CHANGELOG/CHANGELOG-3.2.md) і [v3.3.4](https://github.com/etcd-io/etcd/blob/main/CHANGELOG/CHANGELOG-3.3.md) виправляють перезавантаження TLS, коли [поле SAN сертифіката містить лише IP-адреси, але не доменні імена](https://github.com/etcd-io/etcd/issues/9541). Наприклад, учасник налаштований з CSR (з `cfssl`) наступним чином:
...
[auth]: /docs/v3.6/op-guide/authentication/
