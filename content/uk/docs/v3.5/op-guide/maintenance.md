...
Якщо період зберігання перевищує 1 годину, etcd виконує ущільнення щогодини, зберігаючи повний період зберігання. Якщо період зберігання становить 1 годину або менше, etcd виконує ущільнення з інтервалом, що відповідає періоду зберігання.

Наприклад, при параметрі `--auto-compaction-retention=10h` etcd чекає 10 годин до першого ущільнення, а потім виконує ущільнення щогодини:
...
Встановлення квоти на дисковий простір також допомагає помʼякшити атаки типу «відмова в обслуговуванні» (DoS), спрямовані на вичерпання дискового простору. `etcd` перевіряє квоту як на рівні API (раннє відхилення), так і на рівні Apply (здіймання тривоги `NOSPACE`), обмежуючи вплив надмірних запитів на запис на ресурси кластера. Дивіться примітку в кінці цього розділу для деталей про те, як взаємодіють ці дві перевірки.
