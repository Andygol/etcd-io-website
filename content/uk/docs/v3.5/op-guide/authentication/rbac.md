...
## Обсяг безпеки автентифікації {#security-scope-of-authentication}

Коли автентифікацію увімкнено за допомогою `etcdctl auth enable`, вона захищає операції V3 gRPC API (get, put, delete, watch тощо).

HTTP-точки доступу `/metrics` та `/health` працюють на окремому обробнику і **не** захищені автентифікацією V3 RBAC. Цей дизайн дозволяє Prometheus та балансувальникам навантаження збирати метрики без вимоги gRPC-автентифікації, зберігаючи при цьому захист даних ключ-значення.

Щоб захистити ці точки доступу спостережуваності:

- Увімкніть mTLS за допомогою `--cert-file`, `--key-file` та `--client-cert-auth`
- Або привʼяжіть метрики до приватного інтерфейсу за допомогою `--listen-metrics-urls`
- Або використовуйте мережеві політики/правила файерволу для обмеження доступу
...
etcd не підтримує автентифікацію з порожнім паролем за допомогою `--user username:`. Наприклад, користувач, створений з порожнім паролем, наприклад `etcdctl user add anonymous:''`, не може пройти автентифікацію через запити імені користувача/пароля, і запити на кшталт `etcdctl --user anonymous: get foo` завершуються помилкою `user name is empty`.
{{% /alert %}}
