...
---
Наступний посібник показує, як запустити etcd з Docker за допомогою [статичного процесу початкового завантаження](/docs/v3.8/op-guide/clustering/#static).
...
Запустіть останню версію etcd (`{{< param git_version_tag >}}` на час написання):
...
Контейнер випуску etcd стандартно не включає сертифікати кореневих центрів сертифікації. Щоб використовувати HTTPS із сертифікатами, довіреними кореневим центром сертифікації (наприклад, для виявлення), змонтуйте теку сертифікатів у контейнер etcd:
