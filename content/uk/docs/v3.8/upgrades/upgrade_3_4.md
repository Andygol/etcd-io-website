...
#### Зробити `ETCDCTL_API=3 etcdctl` стандартним {#make-etcdctl_api3-etcdctl-default}
...
#### Зробити `etcd --enable-v2=false` стандартним {#make-etcd---enable-v2false-default}
...
#### Застаріла помилка `grpc.ErrClientConnClosing` {#deprecated-grpcerrclientconnclosing-error}
...
#### Вимагати `grpc.WithBlock` для клієнтського підключення {#require-grpcwithblock-for-client-dial}
...
#### Перейменовано `embed.Config.LogOutput` на `embed.Config.LogOutputs` {#renamed-embedconfiglogoutput-to-embedconfiglogoutputs}
...
#### v3.5 визнає застарілим `capnslog` {#v35-deprecates-capnslog}
...
#### Застарілий прапорець `etcd --debug` (тепер `--log-level=debug`) {#deprecating-etcd---debug-flag-now---log-leveldebug}
...
#### Застаріле поле `pkg/transport.TLSInfo.CAFile` {#deprecated-pkgtransporttlsinfocafile-field}
...
#### Змінено `embed.Config.SnapCount` на `embed.Config.SnapshotCount` {#changed-embedconfigsnapcount-to-embedconfigsnapshotcount}
...
#### Змінено `etcdserver.ServerConfig.SnapCount` на `etcdserver.ServerConfig.SnapshotCount` {#changed-etcdserverserverconfigsnapcount-to-etcdserverserverconfigsnapshotcount}
...
#### Змінено тип `IntervalTree` у пакеті `pkg/adt` {#changed-intervaltree-type-in-package-pkgadt}
...
#### Застаріле `embed.Config.SetupLogging` {#deprecated-embedconfigsetuplogging}
...
Щоб оновити поточне розгортання etcd до 3.4, кластер, що працює, повинен бути 3.3 або вище. Якщо це до 3.3, будь ласка, [оновіться до 3.3](/docs/v3.8/upgrades/upgrade_3_3/) перед оновленням до 3.4.
...
Будь ласка, [завантажте резервну копію знімка](/docs/v3.8/op-guide/maintenance/#snapshot-backup), щоб зробити можливим пониження версії кластера навіть після його повного оновлення.
...
[Завантажте резервну копію знімка](/docs/v3.8/op-guide/maintenance/#snapshot-backup), щоб забезпечити шлях до пониження версії у разі виникнення будь-яких проблем.
