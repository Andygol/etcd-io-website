...
linktitle: Автентифікація у v3
weight: 2500
description: Автентифікація в etcd v3
---

## Чому не використовувати систему автентифікації v2? {#why-not-reuse-the-v2-auth-system}
...
### Неконсистентні дозволи небезпечні для лінеаризованих запитів {#inconsistent-permissions-are-unsafe-for-linearized-requests}
...
Існує відома проблема [#18437](https://github.com/etcd-io/etcd/issues/18437) з простими токенами. У серверах etcd токени розвʼязуються на рівні API, а прості токени є stateful. Процес не захищений лінеаризованою перевіркою, що означає, що член etcd може не завершити обробку попереднього запиту на автентифікацію перед отриманням наступного. У таких випадках член може повернути клієнту помилку "недійсний токен автентифікації". Ця проблема зазвичай рідкісна на вузлі з хорошими мережевими умовами, але може виникнути, якщо є значна затримка. Як обхідний шлях, застосунки можуть реалізувати механізм повторної спроби для обробки цієї помилки.
{{% /alert %}}

### Безпосереднє встановлення JWT-токенів {#directly-setting-jwt-tokens}

На додачу до стандартного потоку RPC `Authenticate()`, etcd підтримує безпосереднє встановлення JWT-токенів на рівні клієнта. Це дозволяє застосункам керувати повним життєвим циклом JWT-токенів поза межами etcd, включаючи генерацію, валідацію та ротацію токенів.

#### Випадок використання та робочий процес {#use-case-and-workflow}

Цей підхід є корисним, коли:

* Окрема система керування токенами (поза межами etcd) обробляє генерацію та життєвий цикл JWT-токенів
* Застосунки отримують підписані заздалегідь JWT-токени через зовнішній механізм (наприклад, змінні середовища, конфігураційний сервіс)
* Життєвим циклом токенів повинен керувати клієнтський застосунок, а не автоматична генерація токенів etcd

Типовий робочий процес:

1. Зовнішній орган (не etcd) генерує підписаний JWT-токен, що містить імʼя користувача та інші вимоги
2. Застосунок отримує підписаний заздалегідь токен та налаштовує клієнт etcd з ним
3. Клієнт надсилає JWT-токен безпосередньо із запитами (без виклику `Authenticate()`)
4. Сервер etcd перевіряє підпис токена за допомогою налаштованого публічного ключа та надає доступ на основі імені користувача в токені
5. Перед тим, як токен закінчиться, застосунок отримує новий токен від зовнішнього органу
6. Застосунок створює нового клієнта з оновленим токеном (оновлення токена потребує перестворення клієнта)

#### Чим відрізняється від стандартної автентифікації {#how-it-differs-from-standard-authentication}

При використанні стандартного потоку `Authenticate()`:

* Клієнт викликає `Authenticate()` з іменем користувача та паролем
* etcd генерує та повертає токен
* Клієнт автоматично використовує цей токен для наступних запитів
* Оновлення токена потребує повторного виклику `Authenticate()`

При безпосередньому встановленні JWT-токенів:

* Клієнт ініціалізується підписаним заздалегідь JWT-токеном
* Клієнт **не** викликає `Authenticate()`
* Токен використовується безпосередньо у всіх запитах
* Клієнтський застосунок відповідає за отримання нових токенів до закінчення терміну дії та керування життєвим циклом клієнта

#### AuthStatus без дійсного токена {#authstatus-without-valid-token}

Для підтримки застосунків, які керують власними JWT-токенами, RPC `AuthStatus` розроблено так, щоб дозволити клієнтам визначити, чи увімкнена автентифікація, та отримати поточну `authRevision`. Це важливо для сценаріїв відновлення, коли токен закінчився, і клієнту потрібна остання ревізія для отримання нового дійсного токена від зовнішнього постачальника токенів.

Без цієї можливості закінчений токен міг би запобігти отриманню клієнтом поточної `authRevision`, що призвело б до взаємного блокування, за якого новий токен не може бути згенеровано.
