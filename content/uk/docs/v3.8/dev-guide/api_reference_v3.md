...
| Поле | Опис | Тип |
| ---- | ---- | --- |
| enabled | enabled вказує, чи кластер увімкнено для зниження версії. | bool |
| targetVersion | targetVersion - це цільова версія зниження. | string |
...
DowngradeVersionTestRequest використовується лише для тестування. Версія в цьому запиті буде прочитана як версія запису WAL. Якщо цільова версія зниження менша за цю версію, то зниження (онлайн) або міграція (офлайн) не є безпечними, тому не повинні бути дозволені.
...
| hash_revision | hash_revision - це ревізія, до якої обчислюється хеш. | int64 |
...
| count | count встановлюється на фактичну кількість ключів у діапазоні, коли запитується. На відміну від Kvs, на нього не впливають обмеження та фільтри (наприклад, Min/Max, Create/Modify, Revisions), і він відображає повну кількість у вказаному діапазоні. | int64 |
...
| ---- | ---- | --- |
| (versionpb.etcd_version_msg) |  | option |
| request | request - це обʼєднання типів запитів, прийнятих транзакцією. | oneof |
...
| revision | revision - це ревізія сховища ключ-значення, коли запит був застосований, і вона не встановлена (тобто 0) у випадку викликів, які не взаємодіють зі сховищем ключ-значення. Для відповідей на прогрес спостереження, header.revision вказує на прогрес. Всі майбутні події, отримані в цьому потоці, гарантовано матимуть вищий номер ревізії, ніж номер header.revision. | int64 |
...
| ---- | ---- | --- |
| (versionpb.etcd_version_msg) |  | option |
| response | response - це обʼєднання типів відповідей, повернутих транзакцією. | oneof |
...
| version | version - це локальна версія сервера, який створив знімок. У кластері з бінарними файлами різних версій кожен учасник може повернути різний результат. Вказує, яку версію сервера etcd слід використовувати при відновленні знімка. | string |
...
| isLearner | isLearner вказує, чи є учасник raft learner. | bool |
| storageVersion | storageVersion - це версія файлу бази даних. Вона може оновлюватися із затримкою відносно цільової версії кластера. | string |
| dbSizeQuota | dbSizeQuota - це налаштована квота зберігання etcd в байтах (значення, передане екземпляру etcd через прапорець --quota-backend-bytes) | int64 |
| downgradeInfo | downgradeInfo вказує, чи існує процес зниження версії. | DowngradeInfo |
...
| canceled | canceled встановлюється на true, якщо відповідь стосується запиту на скасування спостереження або якщо start_revision вже була стиснута. Більше подій не буде надіслано скасованому спостерігачу. | bool |
