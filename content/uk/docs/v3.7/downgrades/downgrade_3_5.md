...
Якщо у вашому кластері увімкнено автентифікацію, поступове зниження версії з 3.5 не підтримується, оскільки 3.5 [змінює формат записів WAL, повʼязаних з автентифікацією](https://github.com/etcd-io/etcd/pull/11943). Ви можете дотримуватися [інструкцій з автентифікації](/docs/v3.7/op-guide/authentication/rbac/), щоб вимкнути автентифікацію та видалити всіх користувачів спочатку.
{{% /alert %}}
...
Різниця базується на версіях 3.5.14 та 3.4.33. Фактична різниця залежатиме від вашої патч-версії, перевірте за допомогою `diff <(etcd-3.5/bin/etcd -h | grep \\-\\-) <(etcd-3.4/bin/etcd -h | grep \\-\\-)` спочатку.
{{% /alert %}}
...
Якщо будь-який член був знижений до 3.4, версія кластера буде знижена до 3.4, і операції будуть сумісні з "3.4". Вам потрібно буде дотримуватися інструкцій [Оновлення etcd з 3.4 до 3.5](/docs/v3.7/upgrades/upgrade_3_5/), щоб відкотитися.

Будь ласка, [завантажте резервну копію знімка](/docs/v3.7/op-guide/maintenance/#snapshot-backup), щоб зробити можливим зниження версії кластера навіть після його повного зниження.
...
[Завантажте резервну копію знімка](/docs/v3.7/op-guide/maintenance/#snapshot-backup), щоб забезпечити шлях для зниження версії у разі виникнення проблем.
