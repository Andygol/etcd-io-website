...
Ідентифікатор оренди `1234abcd` у наведеній вище команді належить до ідентифікатора, повернутого при створенні оренди на 10 секунд. Цей ідентифікатор потім може бути прикріплений до ключа.
{{% /alert %}}
...
{{% alert title="Примітка" color="info" %}}
`foo3` виключено, оскільки діапазон охоплює напіввідкритий інтервал `[foo, foo3)`, виключаючи `foo3`.
{{% /alert %}}
...
Ось команда для діапазону всіх ключів з префіксом `foo` за допомогою RPC [`RangeStream`](/docs/v3.7/learning/api/#rangestream). Результат ідентичний унарному `Range`:
...
`--stream` не підтримує `--order`, `--sort-by` або фільтри ревізій.
...
{{% alert title="Примітка" color="info" %}}
Номер ревізії у відповіді на повідомлення про прогрес є ревізією з локального вузла сервера etcd, до якого підключений потік спостереження. Якщо цей вузол розділений і не є частиною кворуму, ця ревізія прогресу може бути нижчою, ніж ревізія, повернута кворумним читанням з нерозділеного вузла сервера etcd.
{{% /alert %}}
...
{{% alert title="Примітка" color="info" %}}
Поточну ревізію сервера etcd можна знайти за допомогою команди get на будь-якому ключі (наявному або відсутньому) у форматі json. Приклад показано нижче для mykey, який не існує на сервері etcd:
{{% /alert %}}
