...
### TLS keyUsage та extendedKeyUsage сертифікатів {#tls-certificate-keyusage-and-extendedkeyusage}

Під час генерації X.509-сертифікатів для захисту транспорту etcd сертифікати повинні містити відповідні поля `keyUsage` та `extendedKeyUsage` залежно від їхньої ролі. etcd покладається на бібліотеки `crypto/tls` та `crypto/x509` з Go для перевірки сертифікатів, які застосовують ці використання під час TLS-рукостискання.

Наступна таблиця підсумовує рекомендовані використання для типових ролей сертифікатів:

| Роль сертифіката | keyUsage | extendedKeyUsage |
| ---------------- | -------- | ---------------- |
| Сервер (клієнт-сервер) | digitalSignature, keyEncipherment | serverAuth |
| Клієнт | digitalSignature, keyEncipherment | clientAuth |
| Вузол (сервер-сервер) | digitalSignature, keyEncipherment | serverAuth, clientAuth |

Примітки:

- Коли увімкнено `--peer-client-cert-auth`, сертифікати вузлів використовуються для взаємного TLS між учасниками etcd, тому вимагають обидва `serverAuth` та `clientAuth`.
- Клієнтські сертифікати, що використовуються з `--client-cert-auth`, повинні містити `clientAuth`.
...
Для випадків, коли потрібне шифрування звʼязку, але не автентифікація, etcd підтримує шифрування своїх повідомлень за допомогою автоматично згенерованих самопідписних сертифікатів. Це спрощує розгортання, оскільки немає потреби керувати сертифікатами та ключами поза etcd. Налаштуйте etcd для використання самопідписних сертифікатів для клієнтських і вузлових зʼєднань за допомогою прапорців `--auto-tls` та `--peer-auto-tls`:
...
[auth]: /docs/v3.7/op-guide/authentication/
